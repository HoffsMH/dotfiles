...
var RescueTimeWebSocket = {
  ...
  getMessageCallback: function(msg){
    this.util._log('WebSocket message received', JSON.stringify(msg));
    var self = this,
  ...
  redirect: function(msg){
    var self = this;
    this.util._log('WebSocket FocusTime redirect called', JSON.stringify(msg));
    // for focustime
    if(msg.hasOwnProperty('url') && msg.hasOwnProperty('block_url')){
      ...
      var matchString = '*://*.' + parsedUrl.domain + '/*',
          opts = {
            currentWindow: true,
            active: true,
            url: matchString
      ...
      this.util._log('WebSocket FocusTime url string to match: ', matchString);

      chrome.tabs.query(opts, function(results){
        if(results.length && results[0].hasOwnProperty('id')){
          self.util._log('WebSocket FocusTime updating tab: ', blockUrl);
